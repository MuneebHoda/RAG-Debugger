...
- Sealed autonomous candidates before executing generated code, isolated quality checks in a credential-free disposable job, added fresh publisher-side revalidation by immutable artifact ID and digest, bound canonical builder context and every candidate byte into attestations, enforced a fail-closed schema subset and exact per-step App-token scope, and added adversarial artifact-boundary fixtures.
