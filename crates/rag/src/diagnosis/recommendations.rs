...
fn recommendation_for(failure: &DiagnosisFailure) -> DiagnosisRecommendation {
    let (code, priority, area, title, rationale, action) = match failure.code {
        ...
        DiagnosisFailureCode::AnswerabilityGap => (
            ...
            DiagnosisRecommendationPriority::Critical,
            DiagnosisRemediationArea::CorpusCoverage,
            "Restore direct answer support",
            "Retrieved candidates do not contain body text that directly supports the question.",
            "Add or select source content that explicitly answers the question, then preserve the unsupported query as an eval case.",
        ),
        DiagnosisFailureCode::SemanticOnlyMatch => (
            "tighten_semantic_candidates",
            DiagnosisRecommendationPriority::High,
            DiagnosisRemediationArea::RetrievalMode,
            "Tighten semantic candidate selection",
            "Semantic similarity promoted candidates without direct body-text support.",
            "Compare lexical and hybrid modes, then tune semantic candidate thresholds against an eval case for this question.",
        ),
        DiagnosisFailureCode::MetadataOnlyMatch => (
            "separate_metadata_from_answer_evidence",
            DiagnosisRecommendationPriority::High,
            DiagnosisRemediationArea::MetadataFilters,
            "Separate metadata from answer evidence",
            "Path, section, or metadata relevance was mistaken for answer support.",
            "Keep metadata signals for candidate retrieval, but require matching chunk body evidence before citation.",
        ),
