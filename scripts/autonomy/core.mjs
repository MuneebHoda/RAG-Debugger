...
export function policyIntroducedByRepositoryPush({
  policy,
  beforeSha,
  eventName,
  ref,
  ...
  if (!commitShaPattern.test(beforeSha)) return false;
  try {
    execFileSync("git", ["cat-file", "-e", `${beforeSha}^{commit}`], {
      cwd: repositoryRoot,
      stdio: "ignore",
    });
    ...
    return isReviewedBootstrap({
      eventName,
      ref,
      beforePolicyPresent: policyEntry.trim().length > 0,
...
function sanitizePullRequestText(value) {
  return String(value)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/@/g, "&#64;");
...
export function proposalBody(proposal) {
  return [
    "## Summary",
    sanitizePullRequestText(proposal.summary),
    "## Repository Evidence",
    ...
    sanitizePullRequestText(proposal.architecture),
    "## Acceptance Criteria",
    markdownList(proposal.acceptance_criteria),
    "## Tests",
    markdownList(proposal.tests),
    "## Security And Privacy",
    sanitizePullRequestText(proposal.security),
    "## Performance",
    sanitizePullRequestText(proposal.performance),
    "## Risks",
    markdownList(proposal.risks),
    "## Rollback",
    sanitizePullRequestText(proposal.rollback),
...
export function builderPullRequestBody(output, context) {
  return [
    ...
    "## Summary",
    sanitizePullRequestText(output.summary),
    "## Implementation",
    ...
    "## Rollback",
    sanitizePullRequestText(output.rollback),
    "## Follow-Ups",
