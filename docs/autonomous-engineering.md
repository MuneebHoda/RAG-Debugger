...
Only the fresh publisher receives a short-lived repository GitHub App token. Before the token exists, it downloads the original artifact by the validator's ID, verifies its digest, checks out the exact base without persisted credentials, revalidates all schemas and policy, recalculates every attested hash, and reapplies the candidate deterministically. It runs no candidate build, test, hook, package script, or binary. The App token is minted only after that revalidation, and only trusted base-SHA publication code runs afterward. Every App-token action is pinned and step-locally restricted to exactly `MuneebHoda/RAG-Debugger`; an unrelated workflow expression cannot satisfy that rule. The action must omit `skip-token-revoke`, preserving automatic revocation after each job.

The workflows default the ordinary `GITHUB_TOKEN` to `contents: read`. Only preflight jobs explicitly add read-only issue and pull-request access for inventory and authorization checks. Every job that can mint an App token, including issue claim/block operations and final publishers, explicitly sets its ordinary token to exactly `contents: read`, which is required for the private-repository checkout; no issue, pull-request, workflow, Actions, deployment, package, security-event, OIDC, or repository-write permission is available before the App token is minted. All publication writes use only the repository-restricted App token.
