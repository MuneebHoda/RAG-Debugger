...
The API and Vite app now have hardened production images plus an opt-in Postgres 17 production-parity stack. Hosted/test migrations run through the packaged explicit command while local startup retains automatic migration. The immutable browser bundle reads a separate public runtime config. Trusted main commits publish a full-SHA GHCR API digest and checksummed web/migration/SBOM bundle with GitHub provenance; publication does not deploy. Issue #105 adds a [machine-checked infrastructure desired-state contract and maintainer provisioning checklist](infrastructure-foundations.md), but no external resource is claimed until the provider checklist is completed. `/healthz` reports process liveness and `/readyz` pings configured storage. CORS permits one configured credentialed web origin. The API ignores forwarded headers and emits privacy-safe startup and trace-ingestion events, but does not yet provide hosted JSON request telemetry, deployment automation, verified backups, or provisioned provider resources.
...
- **#104:** implemented trusted immutable GHCR/web publication, SBOM, provenance, scanning, attestations, and release manifest.
- **#105:** implemented the checked-in desired-state and governance contract for separate Cloudflare/Render staging and production resources, regions, domains, one concrete two-host multi-domain Access application per environment with eager cookies and `OPTIONS`-only bypass, tunnels, databases, roles, secrets, quotas, and provider configuration. The issue remains open until the maintainer completes and records the external provisioning checklist.
