...
The API and Vite app now have hardened production images plus an opt-in Postgres 17 production-parity stack. Hosted/test migrations run through the packaged explicit command while local startup retains automatic migration. The immutable browser bundle reads a separate public runtime config. Trusted main commits publish a full-SHA GHCR API digest and checksummed web/migration/SBOM bundle with GitHub provenance; publication does not deploy. `/healthz` reports process liveness and `/readyz` pings configured storage. CORS permits one configured credentialed web origin. The API ignores forwarded headers and emits privacy-safe startup and trace-ingestion events, but does not yet provide hosted JSON request telemetry, deployment automation, backups, or provider resources.
