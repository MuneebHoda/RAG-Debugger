...
Trace builders require a saved retrieval response and include ranked evidence plus the latest rerun comparison. Eval builders include gate outcomes, mode metrics, privacy-safe aggregate provenance, expected/retrieved/missing evidence IDs, failed cases, and the previous compatible experiment when one exists. CI builders add branch, commit, base/head refs, config label, gate metrics, failure labels, recommendations, and the full Eval Lab v2 regression comparison when available, including compatibility, changed fields, newly failed cases, and recovered cases. Legacy stored CI runs without the additive comparison field still use their aggregate regression summary.

Eval regression context is metadata-first. Metadata-only reports may include experiment/baseline IDs, provenance schema and aggregate fingerprints, corpus/chunk counts, embedding configuration/index identity, compatibility, changed field names, gate transitions, metric deltas, failure-label codes, and newly failed or recovered case counts. They must not include provenance document checksums, raw queries, document paths, section titles, snippets, corpus text, or embedding vectors. A non-compatible explicit baseline adds a warning finding so directional results cannot be mistaken for a normal regression.
