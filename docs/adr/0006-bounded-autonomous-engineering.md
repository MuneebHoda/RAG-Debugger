...
A disposable quality job downloads that exact artifact by ID, verifies the digest, reapplies it to the exact base, and runs the complete gate with no write credential, App token, OIDC permission, publication secret, or artifact promotion. A separate fresh publisher downloads the original artifact, repeats schema, authorization, policy, hash, size, secret, path, and exact-base validation, and executes no candidate code. Every job that can mint an App token explicitly limits its ordinary `GITHUB_TOKEN` to `contents: read` for checkout. Only after revalidation does the publisher mint a short-lived token restricted to `MuneebHoda/RAG-Debugger`, with automatic end-of-job revocation preserved, and run trusted publication code.
