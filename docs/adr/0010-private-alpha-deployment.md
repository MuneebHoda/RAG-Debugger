...
Each environment uses one Cloudflare Access self-hosted multi-domain application containing both concrete web and API hostnames. Wildcard hostnames are forbidden. Its [Eager redirect cookie](https://developers.cloudflare.com/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/#multi-domain-applications) setting is enabled so the first successful login at the web hostname preemptively issues the hostname-specific `CF_Authorization` cookies for both hosts before the SPA starts API traffic. Users do not visit or authenticate to the API hostname separately.

Browsers omit cookies from CORS preflight requests, so the Access application uses Cloudflare's narrowly scoped [bypass-OPTIONS-to-origin behavior](https://developers.cloudflare.com/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/cors/#bypass-options-requests-to-origin). Only `OPTIONS` bypasses Access; API-host preflights still reach Axum only through the private tunnel, and its exact credentialed CORS policy remains authoritative for the approved web origin. The API remains unreachable at a provider-default hostname, no non-`OPTIONS` request bypasses Access, and CORS is never widened to `*`.
...
Cloudflare Access is a default-deny alpha admission layer, not CorpusLab authorization. Its authorization cookie only proves Access admission. CorpusLab local authentication, workspace sessions, roles, and scoped API keys remain authoritative inside the application; protected API routes still require a CorpusLab session after Access succeeds. Automation uses a separately scoped Access service token plus the existing CorpusLab API key. The connector and API are private so bypassing Access through a provider default hostname is not possible.
