...
Privacy is enforced after bounded parsing and before persistence or secondary use. Metadata mode stores no content, snippets mode retains only explicit bounded evidence snippets, and full-local content cannot enter Eval Lab or reports until those models preserve ingestion provenance. OTLP is metadata-only until the project model has a separate explicit trusted retention setting, so original span names are replaced by canonical operation labels while span kind remains structural metadata. Imported identities merge atomically by workspace, project, source, and external trace ID. PostgreSQL uses an additive partial unique index and transaction lock; memory uses its existing mutex. Mapper versions may advance on incremental deliveries and update serialized and relational values atomically.
