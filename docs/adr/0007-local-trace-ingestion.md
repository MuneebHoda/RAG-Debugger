...
Privacy is enforced after bounded parsing and before persistence or secondary use. Metadata mode stores no content, snippets mode retains only explicit bounded evidence snippets, and OTLP is metadata-only until the project model has a separate explicit trusted retention setting. Original OTLP span names are replaced by canonical operation labels while span kind remains structural metadata. Full-local native traces may create local Eval cases only through a workspace-authorized source lookup and exact retained-query match. Eval provenance is derived server-side, stored additively on the case, copied into experiment snapshots, and used to reject CI and report creation. Imported identities merge atomically by workspace, project, source, and external trace ID. PostgreSQL uses an additive partial unique index and transaction lock; memory uses its existing mutex. Mapper versions may advance on incremental deliveries and update serialized and relational values atomically.
