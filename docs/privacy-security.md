...
Each environment uses one default-deny Cloudflare Access multi-domain application listing its concrete app and API hostnames. Eager redirect cookies issue the API host's Access cookie during the initial app-host login, so browser API traffic does not require a second authentication or manual API visit. This Access cookie proves only private-alpha admission and never replaces the separate host-only `Secure`, `HttpOnly`, `SameSite=Lax`, `__Host-` CorpusLab session. Only unauthenticated `OPTIONS` preflights may bypass Access; API-host preflights still traverse the private tunnel, the exact credentialed Axum CORS allowlist rejects every origin except the matching web host, and all non-`OPTIONS` API traffic remains Access-gated.
