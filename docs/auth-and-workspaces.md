...
API-key creation returns the full secret once; list responses expose only the display prefix, scope, creation time, last-use time, and revocation time. Revocation requires the authenticated workspace in both the API handler and repository query, so a key UUID from another workspace is indistinguishable from a missing key.
