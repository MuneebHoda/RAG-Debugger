...
- Workspace isolation contracts run against MemoryStore and migrated Postgres for corpus evidence, source/document reads, retrieval candidates, datasets, cases, experiments, and legacy eval runs. A temporary-database migration test verifies singleton ownership backfill and ambiguous multi-workspace quarantine.
