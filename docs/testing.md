...
Fixtures cover model/effort pinning, the fail-closed JSON Schema subset, duplicate proposals, sanitized planner inventory, issue ordering, deterministic branch names, protected/sensitive/artifact paths, size approvals, exact changed-file declarations, pre-artifact secret rejection, path traversal, control-character paths, and symlinks. Static workflow checks enforce immutable action pins, per-step exact App-token scope, read-only default permissions, diagnostic-only dispatch, disabled schedule gates, one model call, lifecycle pause checks, pre-execution immutable upload, disposable credential-free quality, publisher-side revalidation, workspace-write plus `drop-sudo`, and the absence of automatic merge/ready/close operations.

Tests never call Codex, consume paid tokens, mutate repository settings, apply labels, create branches, or open issues/PRs. The Issue #27 fixture creates a temporary Git repository and exercises the real reviewed event, canonical context, trusted bootstrap capability, claimability, candidate capture, immutable sealing, exact-base publisher revalidation, sensitive-path policy, attestation, structural checks, and deterministic draft-PR payload. Adversarial fixtures mutate disposable quality copies, context, manifest, output, attestation, candidate files, artifact identity, protected/sensitive paths, secrets, size, traversal, and symlink destinations. They also prove publication depends on both validation and quality, publisher tokens follow revalidation, no quality artifact can replace the original, and bootstrap completion remains publication-only. Live validation remains the reviewed Issue #27 bootstrap after App, secrets, variables, and budget controls are configured.
