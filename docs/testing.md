...
- Eval Lab API tests for dataset CRUD, case create/update/delete, legacy case backfill, bounded experiment execution with inclusive-limit and no-partial-persistence coverage, cross-mode experiments, immutable privacy-safe provenance serialization, strict/explicit experiment comparison, regression history, trend summaries, gate evaluation, failure diagnosis, two-workspace evidence/eval isolation, and golden dataset export/import round trips, dry-run tokens, modes, versions, privacy, and validation failures.
...
- API key and CI eval tests for one-time secret generation, hashed storage, scoped authorization, workspace-scoped revoke behavior, bounded CI/build provenance metadata, manual/CI experiment-limit parity, compatible baseline preference, incompatible exclusion, CI run and Eval Lab v2 regression persistence, gate failure status, and both branches of `fail_on_gate`.
- Provenance engine tests for canonical fingerprint stability, map/source/chunk/mode order independence, corpus/document checksum and path changes, scoring-relevant chunk text/section/quality changes, chunking changes, chunk-set changes, embedding provider/model/dimension/index changes, scoring/filter/dataset changes, legacy handling, and explicit cross-configuration compatibility. A forced PostgreSQL race blocks between source/document and candidate/embedding phases, applies concurrent mutations, and proves one snapshot cannot mix pre- and post-mutation state.
...
- Audit-report builder tests for deterministic trace/eval/CI output, privacy-safe aggregate provenance, compatibility warnings and changed fields, metadata redaction, bounded snippets, rerun and regression context, recommendation deduplication, and invalid trace sources.
...
DATABASE_URL=postgres://postgres:postgres@localhost:5432/rag_debugger cargo test -p rag-debugger-storage --test eval_workspace_contract postgres_ci_baseline_lookup_reaches_past_one_hundred_incompatible_runs -- --ignored
DATABASE_URL=postgres://postgres:postgres@localhost:5432/rag_debugger cargo test -p rag-debugger-storage --test eval_workspace_contract postgres_golden_dataset_import_is_atomic_and_workspace_scoped -- --ignored
...
- Quality tests for the overview, focused dataset case management, experiment controls, gate-first result view, mode metrics, failure diagnosis, and golden dataset full-export warning disclosure plus dry-run-before-apply import controls.
...
- Settings tests for CI API key onboarding, one-time secret display, scope, creation/last-use metadata, listing, and revoke behavior.
- CI Gates tests for linked run history, recoverable detail errors, revision/config metadata, thresholds, failed metrics/cases, full Eval Lab v2 regression categories, report actions, and GitHub Actions setup copy.
...
- Eval Lab UI tests verify provenance and legacy states, native-details accessibility, trend cards, dataset experiment history, strict automatic selection, explicit cross-configuration selection, compatibility warnings/changed fields, experiment and CI-run regression panels, no-baseline states, failed-case diagnosis, and audit-report actions without duplicating backend regression logic. Real Playwright coverage creates a scoped CI key and failing dataset, submits the authenticated gate, opens the saved failure, and generates its metadata-only report.
