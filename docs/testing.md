...
- Workspace isolation contracts run against MemoryStore and migrated Postgres for corpus evidence, embedding status/writes, retrieval candidates/runs, traces, datasets, cases, immutable experiment provenance, duplicate experiment IDs, and legacy eval runs. Two-workspace API tests cover trace creation/list/detail/rerun/report paths, Overview counts, and embedding mutation. A temporary-database migration test verifies singleton ownership backfill and ambiguous multi-workspace quarantine for projects, Eval records, retrieval runs, and traces.
