...
The repository contains the hardened production API image, static web artifact, public runtime-config boundary, explicit packaged migration command, and opt-in production-parity Compose stack documented in [Production Artifacts](production-artifacts.md). Trusted publication is documented in [Artifact Publication](artifact-publication.md), while the checked [Private-Alpha Infrastructure Foundations](infrastructure-foundations.md) define the separate provider resources and maintainer provisioning boundary without claiming that external infrastructure exists. Deployment workflows, hosted observability, and recovery runbooks remain owned by issues #106–#108. Hosted `staging` and `production` configuration fails before startup when origins, TLS database configuration, secure cookie namespace, release identity, local embedding boundary, or upload limits are unsafe.
