...
`EvidenceRepository` separates direct expected-evidence resolution from bounded candidate search. The API caps submitted ID work before repository access, performs fixed direct-resolution calls, and uses typed `Browse`, `ExactId`, or `Text` search modes rather than storage-specific string interpretation. Requested IDs retain stable request order and sit outside candidate limits. Postgres uses primary-key equality paths, normalized browse indexes, and separately bounded trigram match categories. MemoryStore resolves IDs through hash indexes, browses synchronized ordered document/chunk indexes, and retains only bounded top-k candidates during linear substring search. Evidence responses expose 280-character UTF-8-safe previews rather than complete chunk bodies. Broader workspace-isolation hardening remains Issue #68 and is not part of this boundary.
