...
[Private-Alpha Infrastructure Foundations](infrastructure-foundations.md) now define a checked desired state for two Direct Upload Pages projects, two exact two-host Access applications, separate tunnels/connectors, protected and network-isolated Render environments, private digest-selected API services, Postgres 17 instances, database roles, GitHub Environments, secret locations, resource ceilings, and the operator provisioning checklist. The repository contains no operator domain, provider ID, database URL, allowlist, or credential and makes no claim that those external resources exist. Issue #105 remains open until a maintainer completes the provider checklist with sanitized evidence; no deployment workflow is enabled.
