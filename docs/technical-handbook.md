...
A fresh non-executing publisher downloads the original artifact, verifies the ID-bound digest, checks out the exact base, and repeats trusted validation and deterministic application. Only then does it receive a short-lived App token for repository contents, issues, and draft pull requests. Every token step is pinned and restricted to exactly `MuneebHoda/RAG-Debugger`.
