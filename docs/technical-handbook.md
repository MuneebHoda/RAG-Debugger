...
Generation runs through the pinned official Codex action with workspace-write and `drop-sudo`, but no GitHub App token. Before candidate code executes, a fresh job validates canonical context, authorization, schema output, paths, size, secrets, and every file hash, then directly uploads one immutable sealed file and propagates its artifact ID and digest. A disposable credential-free job downloads that artifact by ID and runs Rust/web quality, Playwright, documentation, migrations, Postgres contracts, and Cargo Deny without uploading publication material.

A fresh non-executing publisher downloads the original artifact, verifies the ID-bound digest, checks out the exact base, and repeats trusted validation and deterministic application. Every App-token-bearing job explicitly limits its ordinary `GITHUB_TOKEN` to `contents: read`; publication receives no issue, pull-request, workflow, deployment, package, security-event, OIDC, or write permission through that token. Only then does it receive a short-lived App token for repository contents, issues, and draft pull requests. Every token step is pinned and restricted to exactly `MuneebHoda/RAG-Debugger`, and automatic token revocation cannot be disabled.
...
Candidate and artifact files use bounded no-follow handles, post-read identity/version checks, canonical control files, a fail-closed schema subset, and immutable byte snapshots for scanning, hashing, sealing, validation, and publication. Bootstrap authorization fails closed when the pre-push commit cannot be read, and its regression tests and fixtures are protected from autonomous edits. The publisher permits only typed issue and commit path segments on the canonical HTTPS GitHub API origin and refuses redirects. See `docs/autonomous-engineering.md` and ADR 0006 for setup, privacy, cost, rotation, audit, pause, and rollback.

Deterministic tests run through `just autonomy-check` and require no model credential or live GitHub mutation. The fixture suite includes an end-to-end sealed Issue #27 publication-eligibility path and negative bootstrap, artifact-identity, quality-mutation, context, sensitive/protected-path, file-race, integrity, destination, credential, and retry cases.
