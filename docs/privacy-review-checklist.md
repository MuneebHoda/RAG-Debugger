...
Metadata-only reports include only aggregate fingerprints, counts, embedding configuration/index identity, compatibility, and fixed changed-field names. They exclude per-document checksums and all content fields. Provenance-build failures emit only a generated correlation ID, opaque workspace/dataset IDs, and the serializer error produced from already-redacted provenance inputs; no raw corpus content, query, path, vector, request body, or credential reaches that event. Tests serialize provenance and reports with synthetic secret markers and assert that raw query/document content and document checksums do not enter provenance/report outputs respectively. Legacy experiments remain readable and are never silently classified as compatible. Rollback removes the additive producer/UI logic; stored JSON remains readable because provenance is optional.
