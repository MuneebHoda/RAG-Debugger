...
The autonomous planner sends public repository source plus bounded, sanitized issue metadata used only for duplicate detection and prioritization; that inventory remains untrusted and is never treated as instructions. The builder sends public repository source plus the sanitized title and body of exactly one authorized, maintainer-approved issue. Both use the pinned Codex GitHub Action. Comments, reviews, commit messages, hidden HTML, external-link contents, secret-like values, and CorpusLab runtime/customer data are outside the trusted input. The workflows are not connected to product databases, workspaces, documents, chunks, embeddings, retrieval queries, traces, reports, sessions, API keys, headers, cookies, or deployment secrets.

Codex receives the dedicated OpenAI project credential through the action's protected proxy and `drop-sudo` path but never receives the GitHub App publication token. Trusted validation seals the exact candidate before candidate code executes. Disposable quality receives no write credential, App token, OIDC permission, publication secret, or artifact-promotion path. A fresh publisher downloads the original artifact by immutable ID, verifies its digest, and repeats all trusted validation before minting its repository-only App token. The App has contents, issues, and pull-request permissions only; it has no Actions, secrets, deployments, administration, or ruleset-bypass access.
...
The Issue #27 trial may change only the exact sensitive CI-key, Settings, report, and privacy-document files recorded in trusted policy. Its capability is bound to the policy-introducing `main` event and revalidated at every write boundary; it does not authorize runtime customer data, general authentication/session code, storage ownership, migrations, dependencies, environment configuration, or unrelated privacy behavior. Candidate publication uses immutable, secret-scanned byte snapshots and an exact HTTPS GitHub API origin with redirects disabled.
