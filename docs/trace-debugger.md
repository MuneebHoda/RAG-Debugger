...
`POST /api/v1/traces/ingest` accepts versioned native traces, and `POST /api/v1/otel/v1/traces` accepts authenticated OTLP/HTTP protobuf with a required `x-corpuslab-project-id` header that is verified inside the key's workspace. Imported detail shows source, mapping status, external identity, versions, privacy mode, limitations, permitted evidence, span kind, and an accessible parent/child hierarchy. Metadata/snippet imports display canonical operation names because original span names may contain content; full-local imports display the validated original name. Metadata/snippet imports explain why Eval conversion is unavailable. Full-local native imports with a retained query offer the existing authorized evidence picker; the resulting case is visibly classified as local-only, evaluates locally, preserves source provenance in experiment results, and is excluded from CI and reports. The server rejects reruns for every imported privacy mode before retrieval or persistence because the originating application owns that execution.
