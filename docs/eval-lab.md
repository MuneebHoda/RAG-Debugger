...
Chunk lookup returns at most 280 Unicode characters in `text_preview` plus `preview_truncated`; it never returns the unrestricted chunk body. Postgres resolves IDs through primary-key joins, browses through normalized-path/document/chunk B-tree indexes, and runs separate bounded text-match categories backed by trigram indexes for source names, paths, section titles, and chunk text. Text-query CPU depends on index match cardinality, while each category and the final response retain only bounded candidates. The API no longer lists every source or loads chunks document by document.

Memory storage keeps a direct chunk-ID index and a bounded top-k heap. Search CPU is `O(D log document_limit + C log chunk_limit)` and temporary candidate memory is `O(document_limit + chunk_limit)`. Browse and text modes still scan the in-memory corpus while holding the consistency lock; the heap bounds retention, not scan CPU. Zero candidate limits return before the lock is acquired.
